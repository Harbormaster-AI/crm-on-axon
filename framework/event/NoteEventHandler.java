/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * CRM Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.event;

import org.axonframework.config.ProcessingGroup;
import org.axonframework.eventhandling.EventHandler;
import org.springframework.stereotype.Component;

import java.util.*;
import java.util.logging.Level;
import java.util.logging.Logger;
import java.util.stream.Collectors;

import com.harbormaster.api.*;
import com.harbormaster.entity.*;
import com.harbormaster.repository.*;

/**
 * ============================================================================
 * NoteEventHandler
 * ============================================================================
 *
 * Event handler for Note as outlined for the CQRS pattern.  All reads related to Note are invoked here
 * and dispersed as an event to be handled elsewhere.
 *
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>CRM Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core CRM capabilities across accounts, contacts, leads, opportunities, pricing, quotes, orders, service cases, activities, and campaigns.</td></tr>
 *          <tr><td>industry</td><td>CRM</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * Services Used:
 *
 *      NoteRepository
 *
 * Events Handled
 *      CreatedNoteEvent
 *      UpdatedNoteEvent
 *      DeletedNoteEvent
 *
 * @author Harbormaster Dev Team
 * ============================================================================
 */
@Component("note-handler")
@ProcessingGroup("note-processing-group")
public class NoteEventHandler {
	
	// core constructor
	NoteEventHandler(EntityManager entityManager) {
        this.entityManager = entityManager;
    }	
	
    /*
     * @param	event CreateNoteEvent
     */
    @EventHandler
    public void handle( CreateNoteEvent event) {
        entityManager.persist(new Note(event.getNoteId(), event.getTitle(), event.getContent(), event.getCreatedAt(), event.getUpdatedAt(), event.getOrganization(), event.getOwner(), event.getAccount(), event.getContact(), event.getOpportunity(), event.getCase(), event.getLead()));
    }

    /*
     * @param	event UpdateNoteEvent
     */
    @EventHandler
    public Note handle( UpdateNoteEvent event) {
    	entityManager.merge(new Note(event.getNoteId(), event.getTitle(), event.getContent(), event.getCreatedAt(), event.getUpdatedAt(), event.getOrganization(), event.getOwner(), event.getAccount(), event.getContact(), event.getOpportunity(), event.getCase(), event.getLead()));
    }
    
    /*
     * @param	event DeleteNoteEvent
     */
    @EventHandler
    public void on( DeleteNoteEvent event) {
    	Note entity = new Note();
   
    	// ------------------------------------------
    	// apply the NoteId
    	// ------------------------------------------
    	entity.setNoteId( event.getNoteId() );
    	
    	// ------------------------------------------
    	// add to an Iterable and delete
    	// ------------------------------------------
    	entityManager.remove( entity );
    }    
    
    // attributes
	private final NoteRepository noteEntityRepository;
    private static final Logger LOGGER 			= Logger.getLogger(NoteEventHandler.class.getName());

}
