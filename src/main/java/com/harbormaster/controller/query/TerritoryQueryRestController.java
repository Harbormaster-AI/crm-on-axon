/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * CRM Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.query;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;
/**
 * <h2>TerritoryQueryRestController as REST API</h2>
 * <p>
 * Implements Spring Controller query CQRS processing for entity Territory.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>CRM Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core CRM capabilities across accounts, contacts, leads, opportunities, pricing, quotes, orders, service cases, activities, and campaigns.</td></tr>
 *          <tr><td>industry</td><td>CRM</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	TerritoryService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateTerritoryCommand</li>
 *      <li></li>UpdateTerritoryCommand</li>
 *      <liDeleteTerritoryCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */
@CrossOrigin
@RestController
@RequestMapping("/TerritoryQuery")
public class TerritoryQueryRestController extends BaseSpringRestController {
	
    /**
     * Handles loading a Territory using a UUID
     * @param		UUID territoryId
     * @return		Territory
     */    
    @GetMapping("/load")
    public Territory load( @RequestParam(required=true) UUID territoryId ) {
    	Territory entity = null;

    	try {  
    		entity = TerritoryService.getTerritoryInstance().getTerritory( new TerritoryFetchOneSummary( territoryId ) );
        }
        catch( Throwable exc ) {
            LOGGER.log( Level.WARNING, "failed to load Territory using Id " + territoryId );
            return null;
        }

        return entity;
    }

    /**
     * Handles loading all Territory business objects
     * @return		Set<Territory>
     */
    @GetMapping("/")
    public List<Territory> loadAll() {                
    	List<Territory> territoryList = null;
        
    	try {
            // load the Territory
            territoryList = TerritoryService.getTerritoryInstance().getAllTerritory();
            
            if ( territoryList != null )
                LOGGER.log( Level.INFO,  "successfully loaded all Territorys" );
        }
        catch( Throwable exc ) {
            LOGGER.log( Level.WARNING,  "failed to load all Territorys ", exc );
        	return null;
        }

        return territoryList;
                            
    }



//************************************************************************    
// Attributes
//************************************************************************
    protected Territory territory = null;
    private static final Logger LOGGER = Logger.getLogger(TerritoryQueryRestController.class.getName());
    
}
