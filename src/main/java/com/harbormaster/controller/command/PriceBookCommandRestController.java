/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * CRM Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>PriceBookCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity PriceBook.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>CRM Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core CRM capabilities across accounts, contacts, leads, opportunities, pricing, quotes, orders, service cases, activities, and campaigns.</td></tr>
 *          <tr><td>industry</td><td>CRM</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	PriceBookService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreatePriceBookCommand</li>
 *      <li>UpdatePriceBookCommand</li>
 *      <liDeletePriceBookCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/PriceBookCommand")
public class PriceBookCommandRestController extends BaseSpringRestController {

	public PriceBookCommandRestController( PriceBookService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a PriceBook.
     * @param		PriceBook	priceBook
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreatePriceBookCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createPriceBook( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a PriceBook.  if no key provided, calls create, otherwise calls save
     * @param		PriceBook priceBook
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdatePriceBookCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdatePriceBookCommand
			// -----------------------------------------------
			completableFuture = service.updatePriceBook(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "PriceBookController:update() - successfully update PriceBook - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a PriceBook entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID priceBookId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeletePriceBookCommand command = new DeletePriceBookCommand( priceBookId );

    	try {
        	PriceBookService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted PriceBook with key " + command.getPriceBookId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Organization on PriceBook
     * @param		command AssignOrganizationToPriceBookCommand
     */     
	@PutMapping("/assignOrganization")
	public void assignOrganization( @RequestBody AssignOrganizationToPriceBookCommand command ) {
		try {
			service.assignOrganization( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Organization", exc );
        }
	}

    /**
     * unassign Organization on PriceBook
     * @param		 command UnAssignOrganizationFromPriceBookCommand
     */     
	@PutMapping("/unAssignOrganization")
	public void unAssignOrganization( @RequestBody(required=true)  UnAssignOrganizationFromPriceBookCommand command ) {
		try {
			service.unAssignOrganization( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Organization", exc );
		}
	}
	

    /**
     * save Entries on PriceBook
     * @param		command AssignEntriesToPriceBookCommand
     */     
	@PutMapping("/addToEntries")
	public void addToEntries( @RequestBody(required=true) AssignEntriesToPriceBookCommand command ) {
		try {
			service.addToEntries( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Entries", exc );
		}
	}

    /**
     * remove Entries on PriceBook
     * @param		command RemoveEntriesFromPriceBookCommand
     */     	
	@PutMapping("/removeFromEntries")
	public void removeFromEntries( 	@RequestBody(required=true) RemoveEntriesFromPriceBookCommand command )
	{		
		try {
			service.removeFromEntries( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Entries", exc );
		}
	}

    /**
     * save Quotes on PriceBook
     * @param		command AssignQuotesToPriceBookCommand
     */     
	@PutMapping("/addToQuotes")
	public void addToQuotes( @RequestBody(required=true) AssignQuotesToPriceBookCommand command ) {
		try {
			service.addToQuotes( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Quotes", exc );
		}
	}

    /**
     * remove Quotes on PriceBook
     * @param		command RemoveQuotesFromPriceBookCommand
     */     	
	@PutMapping("/removeFromQuotes")
	public void removeFromQuotes( 	@RequestBody(required=true) RemoveQuotesFromPriceBookCommand command )
	{		
		try {
			service.removeFromQuotes( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Quotes", exc );
		}
	}

    /**
     * save Orders on PriceBook
     * @param		command AssignOrdersToPriceBookCommand
     */     
	@PutMapping("/addToOrders")
	public void addToOrders( @RequestBody(required=true) AssignOrdersToPriceBookCommand command ) {
		try {
			service.addToOrders( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Orders", exc );
		}
	}

    /**
     * remove Orders on PriceBook
     * @param		command RemoveOrdersFromPriceBookCommand
     */     	
	@PutMapping("/removeFromOrders")
	public void removeFromOrders( 	@RequestBody(required=true) RemoveOrdersFromPriceBookCommand command )
	{		
		try {
			service.removeFromOrders( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Orders", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected PriceBook priceBook = null;
	protected PriceBookService service = null;
    private static final Logger LOGGER = Logger.getLogger(PriceBookCommandRestController.class.getName());
    
}
