/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * CRM Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>TerritoryCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity Territory.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>CRM Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core CRM capabilities across accounts, contacts, leads, opportunities, pricing, quotes, orders, service cases, activities, and campaigns.</td></tr>
 *          <tr><td>industry</td><td>CRM</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	TerritoryService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateTerritoryCommand</li>
 *      <li>UpdateTerritoryCommand</li>
 *      <liDeleteTerritoryCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/TerritoryCommand")
public class TerritoryCommandRestController extends BaseSpringRestController {

	public TerritoryCommandRestController( TerritoryService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a Territory.
     * @param		Territory	territory
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateTerritoryCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createTerritory( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a Territory.  if no key provided, calls create, otherwise calls save
     * @param		Territory territory
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateTerritoryCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateTerritoryCommand
			// -----------------------------------------------
			completableFuture = service.updateTerritory(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "TerritoryController:update() - successfully update Territory - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a Territory entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID territoryId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteTerritoryCommand command = new DeleteTerritoryCommand( territoryId );

    	try {
        	TerritoryService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted Territory with key " + command.getTerritoryId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Organization on Territory
     * @param		command AssignOrganizationToTerritoryCommand
     */     
	@PutMapping("/assignOrganization")
	public void assignOrganization( @RequestBody AssignOrganizationToTerritoryCommand command ) {
		try {
			service.assignOrganization( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Organization", exc );
        }
	}

    /**
     * unassign Organization on Territory
     * @param		 command UnAssignOrganizationFromTerritoryCommand
     */     
	@PutMapping("/unAssignOrganization")
	public void unAssignOrganization( @RequestBody(required=true)  UnAssignOrganizationFromTerritoryCommand command ) {
		try {
			service.unAssignOrganization( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Organization", exc );
		}
	}
	

    /**
     * save Accounts on Territory
     * @param		command AssignAccountsToTerritoryCommand
     */     
	@PutMapping("/addToAccounts")
	public void addToAccounts( @RequestBody(required=true) AssignAccountsToTerritoryCommand command ) {
		try {
			service.addToAccounts( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Accounts", exc );
		}
	}

    /**
     * remove Accounts on Territory
     * @param		command RemoveAccountsFromTerritoryCommand
     */     	
	@PutMapping("/removeFromAccounts")
	public void removeFromAccounts( 	@RequestBody(required=true) RemoveAccountsFromTerritoryCommand command )
	{		
		try {
			service.removeFromAccounts( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Accounts", exc );
		}
	}

    /**
     * save Users on Territory
     * @param		command AssignUsersToTerritoryCommand
     */     
	@PutMapping("/addToUsers")
	public void addToUsers( @RequestBody(required=true) AssignUsersToTerritoryCommand command ) {
		try {
			service.addToUsers( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to add to Set Users", exc );
		}
	}

    /**
     * remove Users on Territory
     * @param		command RemoveUsersFromTerritoryCommand
     */     	
	@PutMapping("/removeFromUsers")
	public void removeFromUsers( 	@RequestBody(required=true) RemoveUsersFromTerritoryCommand command )
	{		
		try {
			service.removeFromUsers( command );
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to remove from Set Users", exc );
		}
	}


//************************************************************************    
// Attributes
//************************************************************************
    protected Territory territory = null;
	protected TerritoryService service = null;
    private static final Logger LOGGER = Logger.getLogger(TerritoryCommandRestController.class.getName());
    
}
