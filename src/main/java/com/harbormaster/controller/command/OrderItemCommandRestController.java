/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * CRM Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>OrderItemCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity OrderItem.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>CRM Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core CRM capabilities across accounts, contacts, leads, opportunities, pricing, quotes, orders, service cases, activities, and campaigns.</td></tr>
 *          <tr><td>industry</td><td>CRM</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	OrderItemService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateOrderItemCommand</li>
 *      <li>UpdateOrderItemCommand</li>
 *      <liDeleteOrderItemCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/OrderItemCommand")
public class OrderItemCommandRestController extends BaseSpringRestController {

	public OrderItemCommandRestController( OrderItemService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a OrderItem.
     * @param		OrderItem	orderItem
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateOrderItemCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createOrderItem( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a OrderItem.  if no key provided, calls create, otherwise calls save
     * @param		OrderItem orderItem
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateOrderItemCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateOrderItemCommand
			// -----------------------------------------------
			completableFuture = service.updateOrderItem(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "OrderItemController:update() - successfully update OrderItem - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a OrderItem entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID orderItemId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteOrderItemCommand command = new DeleteOrderItemCommand( orderItemId );

    	try {
        	OrderItemService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted OrderItem with key " + command.getOrderItemId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Order on OrderItem
     * @param		command AssignOrderToOrderItemCommand
     */     
	@PutMapping("/assignOrder")
	public void assignOrder( @RequestBody AssignOrderToOrderItemCommand command ) {
		try {
			service.assignOrder( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Order", exc );
        }
	}

    /**
     * unassign Order on OrderItem
     * @param		 command UnAssignOrderFromOrderItemCommand
     */     
	@PutMapping("/unAssignOrder")
	public void unAssignOrder( @RequestBody(required=true)  UnAssignOrderFromOrderItemCommand command ) {
		try {
			service.unAssignOrder( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Order", exc );
		}
	}
	
    /**
     * save Product on OrderItem
     * @param		command AssignProductToOrderItemCommand
     */     
	@PutMapping("/assignProduct")
	public void assignProduct( @RequestBody AssignProductToOrderItemCommand command ) {
		try {
			service.assignProduct( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Product", exc );
        }
	}

    /**
     * unassign Product on OrderItem
     * @param		 command UnAssignProductFromOrderItemCommand
     */     
	@PutMapping("/unAssignProduct")
	public void unAssignProduct( @RequestBody(required=true)  UnAssignProductFromOrderItemCommand command ) {
		try {
			service.unAssignProduct( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Product", exc );
		}
	}
	
    /**
     * save PriceBookEntry on OrderItem
     * @param		command AssignPriceBookEntryToOrderItemCommand
     */     
	@PutMapping("/assignPriceBookEntry")
	public void assignPriceBookEntry( @RequestBody AssignPriceBookEntryToOrderItemCommand command ) {
		try {
			service.assignPriceBookEntry( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign PriceBookEntry", exc );
        }
	}

    /**
     * unassign PriceBookEntry on OrderItem
     * @param		 command UnAssignPriceBookEntryFromOrderItemCommand
     */     
	@PutMapping("/unAssignPriceBookEntry")
	public void unAssignPriceBookEntry( @RequestBody(required=true)  UnAssignPriceBookEntryFromOrderItemCommand command ) {
		try {
			service.unAssignPriceBookEntry( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign PriceBookEntry", exc );
		}
	}
	


//************************************************************************    
// Attributes
//************************************************************************
    protected OrderItem orderItem = null;
	protected OrderItemService service = null;
    private static final Logger LOGGER = Logger.getLogger(OrderItemCommandRestController.class.getName());
    
}
