/*
 * Generated by Harbormaster
 *
 *
 * Blueprint:
 * Axon4-Framework-Server
 *
 * Domain Model:
 * CRM Industry Domain Model
 *
 * Do not edit manually.
 ********************************************************
 *  Confidential
 *
 * 2026 
 * All Rights Reserved.
 *
 * This file is subject to the terms and conditions defined in
 * file 'license.txt', which is part of this source code package.
 *
 * Contributors :
 *        - General Release
 */
package com.harbormaster.controller.command;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.*;
import java.util.concurrent.*;
import java.util.logging.Level;
import java.util.logging.Logger;

import com.harbormaster.api.*;
import com.harbormaster.service.*;
import com.harbormaster.entity.*;
import com.harbormaster.exception.*;
import com.harbormaster.projector.*;

import com.harbormaster.controller.*;

/**
 * <h2>OpportunityLineItemCommandRestController as REST API</h2>
 * <p>
 * Implements Spring Controller command CQRS processing for entity OpportunityLineItem.
 * </p>
  * Generated By Harbormaster
 *
 * Generated From
 *
 *      <h3>Blueprint</h3>
 * 			<table>
 *          <tr><td>name</td><td>Axon4-Framework-Server</td></tr>
 *          <tr><td>published</td><td>09/08/2026</td></tr>
 *          <tr><td>design pattern</td><td>CQRS</td></tr>
 *          <tr><td>architecture style</td><td>EventDrivenArchitecture</td></tr>
 *          </table>
 *
 *      <h3>Domain Model</h3>
 *          <table>
 *          <tr><td>name</td><td>CRM Industry Domain Model</td></tr>
 *          <tr><td>description</td><td>HCIDL model capturing core CRM capabilities across accounts, contacts, leads, opportunities, pricing, quotes, orders, service cases, activities, and campaigns.</td></tr>
 *          <tr><td>industry</td><td>CRM</td></tr>
 *          <tr><td>version</td><td>1.0.0</td></tr>
 *          <table>
 *
 * <h3>Services Used</h3>
 *
 *  	OpportunityLineItemService
 *
 * <h3>Produces</h3>
 *
 *      <p>application/json</p>
 *
 * <h3>Commands Accepted</h3>
 *      <ul>
 *      <li>CreateOpportunityLineItemCommand</li>
 *      <li>UpdateOpportunityLineItemCommand</li>
 *      <liDeleteOpportunityLineItemCommand</li>
 *      </ul>
 *
 * <p>@author Harbormaster Dev Team</p>
 */

@CrossOrigin
@RestController
@RequestMapping("/OpportunityLineItemCommand")
public class OpportunityLineItemCommandRestController extends BaseSpringRestController {

	public OpportunityLineItemCommandRestController( OpportunityLineItemService service ) {
		this.service = service;
	}
	
    /**
     * Handles create a OpportunityLineItem.
     * @param		OpportunityLineItem	opportunityLineItem
     * @return		CompletableFuture<UUID> 
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateOpportunityLineItemCommand command ) {
		CompletableFuture<UUID> completableFuture = null;
		try {

			completableFuture = service.createOpportunityLineItem( command );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage(), exc );        	
        }
		
		return completableFuture;
    }

    /**
     * Handles updating a OpportunityLineItem.  if no key provided, calls create, otherwise calls save
     * @param		OpportunityLineItem opportunityLineItem
     * @return		CompletableFuture<Void>
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateOpportunityLineItemCommand command ) {
		CompletableFuture<Void> completableFuture = null;
		try {                        	        
			// -----------------------------------------------
			// delegate the UpdateOpportunityLineItemCommand
			// -----------------------------------------------
			completableFuture = service.updateOpportunityLineItem(command);;
	    }
	    catch( Throwable exc ) {
	    	LOGGER.log( Level.WARNING, "OpportunityLineItemController:update() - successfully update OpportunityLineItem - " + exc.getMessage());        	
	    }		
		
		return completableFuture;
	}
 
    /**
     * Handles deleting a OpportunityLineItem entity
     * @param		command ${class.getDeleteCommandAlias()}
     * @return		CompletableFuture<Void>
     */
    @DeleteMapping("/delete")    
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID opportunityLineItemId  ) {
    	CompletableFuture<Void> completableFuture = null;
		DeleteOpportunityLineItemCommand command = new DeleteOpportunityLineItemCommand( opportunityLineItemId );

    	try {
        	OpportunityLineItemService delegate = service;

        	completableFuture = delegate.delete( command );
    		LOGGER.log( Level.WARNING, "Successfully deleted OpportunityLineItem with key " + command.getOpportunityLineItemId() );
        }
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, exc.getMessage() );
        }
        
        return completableFuture;
	}        
	

    /**
     * save Opportunity on OpportunityLineItem
     * @param		command AssignOpportunityToOpportunityLineItemCommand
     */     
	@PutMapping("/assignOpportunity")
	public void assignOpportunity( @RequestBody AssignOpportunityToOpportunityLineItemCommand command ) {
		try {
			service.assignOpportunity( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Opportunity", exc );
        }
	}

    /**
     * unassign Opportunity on OpportunityLineItem
     * @param		 command UnAssignOpportunityFromOpportunityLineItemCommand
     */     
	@PutMapping("/unAssignOpportunity")
	public void unAssignOpportunity( @RequestBody(required=true)  UnAssignOpportunityFromOpportunityLineItemCommand command ) {
		try {
			service.unAssignOpportunity( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Opportunity", exc );
		}
	}
	
    /**
     * save Product on OpportunityLineItem
     * @param		command AssignProductToOpportunityLineItemCommand
     */     
	@PutMapping("/assignProduct")
	public void assignProduct( @RequestBody AssignProductToOpportunityLineItemCommand command ) {
		try {
			service.assignProduct( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Product", exc );
        }
	}

    /**
     * unassign Product on OpportunityLineItem
     * @param		 command UnAssignProductFromOpportunityLineItemCommand
     */     
	@PutMapping("/unAssignProduct")
	public void unAssignProduct( @RequestBody(required=true)  UnAssignProductFromOpportunityLineItemCommand command ) {
		try {
			service.unAssignProduct( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Product", exc );
		}
	}
	
    /**
     * save PriceBookEntry on OpportunityLineItem
     * @param		command AssignPriceBookEntryToOpportunityLineItemCommand
     */     
	@PutMapping("/assignPriceBookEntry")
	public void assignPriceBookEntry( @RequestBody AssignPriceBookEntryToOpportunityLineItemCommand command ) {
		try {
			service.assignPriceBookEntry( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign PriceBookEntry", exc );
        }
	}

    /**
     * unassign PriceBookEntry on OpportunityLineItem
     * @param		 command UnAssignPriceBookEntryFromOpportunityLineItemCommand
     */     
	@PutMapping("/unAssignPriceBookEntry")
	public void unAssignPriceBookEntry( @RequestBody(required=true)  UnAssignPriceBookEntryFromOpportunityLineItemCommand command ) {
		try {
			service.unAssignPriceBookEntry( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign PriceBookEntry", exc );
		}
	}
	


//************************************************************************    
// Attributes
//************************************************************************
    protected OpportunityLineItem opportunityLineItem = null;
	protected OpportunityLineItemService service = null;
    private static final Logger LOGGER = Logger.getLogger(OpportunityLineItemCommandRestController.class.getName());
    
}
